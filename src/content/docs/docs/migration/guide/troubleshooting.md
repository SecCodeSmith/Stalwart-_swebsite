...
### Vandelay warns that the session advertises a different origin, then every type fails with a 404

A run against a server reachable on a non-default port warns that the session advertises its API, upload and download URLs on an origin other than the one given to `--url`, and the transfer then fails against that origin, typically with a 404 page served by whatever else listens there. A JMAP client must use the URLs the session advertises, so Vandelay follows them rather than rebasing them onto the address it connected to; silently rewriting them would break the deployments that legitimately serve blobs from a separate host. The fault is in the server's own advertised HTTP URL, which omits the port it is actually reachable on. Setting that URL to the full externally reachable address, including the port, resolves it: on Stalwart this is the `http.url` setting on an older source, or `STALWART_PUBLIC_URL` on the new server. Until it is corrected the run stops with exit code 2 at the first type, naming the connection failure, rather than reporting a partial success.
