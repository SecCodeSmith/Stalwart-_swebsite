...
After the session is located, Vandelay uses the API, upload and download URLs the session itself advertises, exactly as they are written, resolving any relative reference against the address the session was fetched from. This is required of a JMAP client and is not negotiable, so the address given to `--url` is only the starting point for discovery, not necessarily the address the rest of the run talks to. When the origin of an advertised URL, meaning its scheme, host and port, differs from the origin of `--url`, Vandelay prints a warning naming both. Serving the API and the blob endpoints from different hosts is a legitimate deployment, so the warning is not an error and the advertised URL is still used; it exists so that a subsequent failure against an unexpected origin is attributable at a glance. The usual cause is a server reachable on a non-default port whose own advertised HTTP URL omits that port, which is a setting to correct on the server rather than in Vandelay.
